...
## RAILS BCrypt, JSON Web Tokens (JWT) 🔐
...
- Don't forget to enable [CORS](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS) in your app. Uncomment the following in [`config/initializers/cors.rb`](./server/config/initializers/cors.rb). Don't forget to change the origins from `example.com` to `*`
...
- Next let's add the routes we'll need for our server. In [`config/routes.rb`](./server/config/routes.rb):
